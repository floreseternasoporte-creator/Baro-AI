...
# calculator imports now handled below in the calc section
...
CALC_PATTERNS = [
    # ── Comandos directos de cálculo ─────────────────────────────
    r"\bcalcula\b", r"\bcalculame\b", r"\bcalcular\b",
    r"\bresuelve\b", r"\bresuelveme\b", r"\bcalculadora\b",
    r"\bcuanto es\b", r"\bcuanto da\b", r"\bcuanto resulta\b",
    r"\bcuanto seria\b", r"\bcuanto son\b", r"\bcuanto equivale\b",
    r"\bcual es el resultado\b", r"\bcual es el valor de\b",
    r"\boperacion\b", r"\bmatematica\b", r"\bmatematico\b",
    r"\bcuanto\b.*\bmas\b", r"\bcuanto\b.*\bmenos\b",
    # ── Operaciones básicas ───────────────────────────────────────
    r"\bsuma\b", r"\bsumar\b", r"\bsumame\b", r"\bsuma de\b",
    r"\bresta\b", r"\brestar\b", r"\brestame\b",
    r"\bmultiplica\b", r"\bmultiplicar\b", r"\bmultiplicacion\b", r"\bmultiplicame\b",
    r"\bdivide\b", r"\bdividir\b", r"\bdivision\b",
    r"\bmodulo\b", r"\bresto de la division\b", r"\bmod\b",
    # ── Potencias y raíces ────────────────────────────────────────
    r"\braiz cuadrada\b", r"\braiz cubica\b", r"\braiz de\b",
    r"\braiz n-esima\b", r"\braiz\b.*\d",
    r"\bpotencia\b", r"\belevado a\b", r"\bala potencia\b",
    r"\bal cuadrado\b", r"\bal cubo\b", r"\bal cuarto\b", r"\bal quinto\b",
    r"\bcuadrado de\b", r"\bcubo de\b",
    r"\bdoble de\b", r"\btriple de\b", r"\bcuadruple de\b",
    r"\bla mitad de\b", r"\bun tercio de\b", r"\bun cuarto de\b",
    r"\bpow\b", r"\b\d+\s*\*\*\s*\d+\b",
    # ── Trigonometría ─────────────────────────────────────────────
    r"\bseno\b", r"\bcoseno\b", r"\btangente\b", r"\bcotangente\b",
    r"\bsecante\b", r"\bcosecante\b",
    r"\barcoseno\b", r"\barcocoseno\b", r"\barctangente\b",
    r"\barcsen\b", r"\barccos\b", r"\barctan\b",
    r"\bsin\s*\(", r"\bcos\s*\(", r"\btan\s*\(",
    r"\btrigonometri\b", r"\bangulo en\b", r"\bradiane\b",
    r"\bconvertir grados\b", r"\bgrados a radianes\b", r"\bradianes a grados\b",
    # ── Logaritmos ───────────────────────────────────────────────
    r"\blogaritmo\b", r"\bln\b", r"\blog\b",
    r"\blog natural\b", r"\blog base\b", r"\blog2\b", r"\blog10\b",
    r"\blogaritmo natural\b", r"\blogaritmo base\b",
    # ── Porcentajes ───────────────────────────────────────────────
    r"\bpor ciento\b", r"\bporciento\b", r"\bporcentaje\b", r"\bporcentaje de\b",
    r"[\d\.]+\s*%\s*de\b", r"\b%\s*de\b",
    r"\bcuanto es el \d+%\b", r"\bque porcentaje\b",
    r"\bporcentaje de cambio\b", r"\bporcentaje de descuento\b",
    r"\bdescuento del\b", r"\brebaja del\b",
    # ── Factorial y combinatoria ──────────────────────────────────
    r"\bfactorial\b", r"\bfact\s*\(", r"\b\d+\s*!\b",
    r"\bcombinaciones\b", r"\bpermutaciones\b",
    r"\bcuantas combinaciones\b", r"\bcuantas permutaciones\b",
    r"\bC\s*\(\d", r"\bP\s*\(\d", r"\bcomb\s*\(", r"\bperm\s*\(",
    # ── Estadística ───────────────────────────────────────────────
    r"\bmedia de\b", r"\bpromedio de\b", r"\bmedia aritmetica\b",
    r"\bpromedio\b", r"\bcalcula el promedio\b",
    r"\bmediana de\b", r"\bmoda de\b",
    r"\bdesviacion estandar\b", r"\bdesviacion tipica\b",
    r"\bvarianza de\b", r"\bestadistica de\b",
    # ── Conversiones de unidades ──────────────────────────────────
    r"\bconvierte\b", r"\bconvertir\b", r"\bconversion\b",
    r"\bconvierte\b.*\ba\b", r"\bpasar\b.*\ba\b",
    r"\bkilometros a millas\b", r"\bmillas a kilometros\b",
    r"\bmetros a pies\b", r"\bpies a metros\b",
    r"\bcelsius a fahrenheit\b", r"\bfahrenheit a celsius\b",
    r"\bkilos a libras\b", r"\blibras a kilos\b",
    r"\bcuantos metros en\b", r"\bcuantos km\b",
    r"\bconvierte\b.*\b(grados|metros|km|kg|lb|millas|pies)\b",
    # ── Temperatura ───────────────────────────────────────────────
    r"\b\d+\s*grados\s*(celsius|fahrenheit|kelvin|centigrados)\b",
    r"\bde celsius a fahrenheit\b", r"\bde fahrenheit a celsius\b",
    r"\bde kelvin a celsius\b", r"\bde celsius a kelvin\b",
    # ── Interés ───────────────────────────────────────────────────
    r"\binteres simple\b", r"\binteres compuesto\b",
    r"\bcapital\b.*\btasa\b", r"\btasa de interes\b",
    r"\bcuanto gano\b.*\binteres\b", r"\bcuanto pago\b.*\binteres\b",
    # ── Geometría ─────────────────────────────────────────────────
    r"\barea del\b", r"\barea de la\b", r"\barea de un\b",
    r"\bperimetro del\b", r"\bperimetro de\b",
    r"\bvolumen del\b", r"\bvolumen de\b", r"\bvolumen de un\b",
    r"\bcirculo\b.*\bradio\b", r"\bcuadrado\b.*\blado\b",
    r"\btriangulo\b.*\blados\b", r"\bsuperficie\b.*\b(esfera|cilindro)\b",
    r"\barea y perimetro\b",
    # ── Ecuaciones ────────────────────────────────────────────────
    r"\becuacion cuadratica\b", r"\becuacion de segundo grado\b",
    r"\bresolver ecuacion\b", r"\bdiscriminante\b",
    r"\bx al cuadrado\b", r"\bx\s*\^?\s*2\b",
    # ── Fibonacci y primos ────────────────────────────────────────
    r"\bfibonacci\s*\(", r"\bnumero fibonacci\b",
    r"\bes primo\b", r"\bnumero primo\b", r"\bprimo\?", r"\bprimalidad\b",
    r"\bfactores primos\b", r"\bfactorizacion prima\b", r"\bfactorizar\b",
    # ── IMC ───────────────────────────────────────────────────────
    r"\bimc\b", r"\bindice de masa corporal\b",
    r"\bcalcula mi imc\b", r"\bmi peso es\b.*\bmi talla es\b",
    # ── Regla de tres ─────────────────────────────────────────────
    r"\bregla de tres\b", r"\bproporcional\b", r"\bsi\b.*\bton\b.*\bcuanto\b",
    # ── Progresiones ──────────────────────────────────────────────
    r"\bprogresion aritmetica\b", r"\bprogresion geometrica\b",
    r"\bsuma de la progresion\b", r"\bn-esimo termino\b",
    # ── MCD / MCM ─────────────────────────────────────────────────
    r"\bmcd\b", r"\bmcm\b", r"\bmaximo comun divisor\b",
    r"\bminimo comun multiplo\b", r"\bdivisor comun\b",
    # ── Expresiones directas ─────────────────────────────────────
    r"^\s*-?[\d\.\,]+\s*[\+\-\*\/x×÷\^%]\s*-?[\d\.\,]+",
    r"\b\d+\s*[\+\-\*\/]\s*\d+\b",
    r"\b\d+\s*x\s*\d+\b",
    r"\b\d+\s*\^\s*\d+\b",
    r"\bsqrt\s*\(", r"\babs\s*\(", r"\bfact\s*\(",
]
...
# ─────────────────────────────────────────────────────────────── #
# Importar funciones avanzadas del calculator                      #
# ─────────────────────────────────────────────────────────────── #
...
# ─────────────────────────────────────────────────────────────── #
# Traducción de palabras a expresiones matemáticas                 #
# ─────────────────────────────────────────────────────────────── #
...
def words_to_expression(text: str) -> str:
    ...
    # Frases de múltiples palabras primero (orden importa)
...
# ─────────────────────────────────────────────────────────────── #
# EXTRACCIÓN INTELIGENTE DE OPERACIONES ESPECIALES                 #
# ─────────────────────────────────────────────────────────────── #

def _try_special_operations(text: str) -> Optional[str]:
    """
    Detecta operaciones especiales en lenguaje natural y devuelve
    un string con la respuesta lista, o None si no aplica.
    """
    # ── Porcentaje ──────────────────────────────────────────────
    pct = re.search(r"([\d\.]+)\s*(?:%|por ciento|porciento)\s*de\s*([\d\.]+)", text)
    if pct:
        a, b = float(pct.group(1)), float(pct.group(2))
        return f"({a}/100)*{b}"  # será evaluada normalmente

    # ── Porcentaje de cambio ────────────────────────────────────
    pct_change = re.search(
        r"(?:porcentaje|cambio|variacion)\s+(?:de cambio\s+)?(?:de|entre|del)?\s*([\d\.]+)\s+a\s*([\d\.]+)", text)
    if pct_change:
        return f"__PCT_CHANGE_{pct_change.group(1)}_{pct_change.group(2)}__"

    # ── Raíz cuadrada ───────────────────────────────────────────
    sqrt = re.search(r"raiz cuadrada de\s*([\d\.]+)", text)
    if sqrt:
        return f"sqrt({sqrt.group(1)})"

    # ── Raíz cúbica ─────────────────────────────────────────────
    cbrt = re.search(r"raiz cubica de\s*([\d\.]+)", text)
    if cbrt:
        return f"cbrt({cbrt.group(1)})"

    # ── Raíz N-ésima ────────────────────────────────────────────
    nroot = re.search(r"raiz\s+(\w+)\s+de\s+([\d\.]+)", text)
    if nroot:
        ord_map = {"cuadrada": 2, "cubica": 3, "cuarta": 4, "quinta": 5,
                   "sexta": 6, "septima": 7, "octava": 8, "novena": 9, "decima": 10}
        n_str = nroot.group(1)
        n = ord_map.get(n_str, None) or (int(n_str) if n_str.isdigit() else None)
        if n:
            return f"nroot({n},{nroot.group(2)})"

    # ── Doble, triple, cuádruple ────────────────────────────────
    double = re.search(r"doble de\s*([\d\.]+)", text)
    if double:
        return f"2*{double.group(1)}"
    triple = re.search(r"triple de\s*([\d\.]+)", text)
    if triple:
        return f"3*{triple.group(1)}"
    quadruple = re.search(r"cuadruple de\s*([\d\.]+)", text)
    if quadruple:
        return f"4*{quadruple.group(1)}"

    # ── Mitad, tercio, cuarto ───────────────────────────────────
    half = re.search(r"(?:la mitad|medio) de\s*([\d\.]+)", text)
    if half:
        return f"{half.group(1)}/2"
    third = re.search(r"un tercio de\s*([\d\.]+)", text)
    if third:
        return f"{third.group(1)}/3"
    quarter = re.search(r"un cuarto de\s*([\d\.]+)", text)
    if quarter:
        return f"{quarter.group(1)}/4"

    # ── Al cuadrado, cubo ───────────────────────────────────────
    squared = re.search(r"([\d\.]+)\s*al cuadrado", text)
    if squared:
        return f"{squared.group(1)}**2"
    cubed = re.search(r"([\d\.]+)\s*al cubo", text)
    if cubed:
        return f"{cubed.group(1)}**3"
    power_n = re.search(r"([\d\.]+)\s*elevado\s*a\s*(?:la\s+)?(\w+)", text)
    if power_n:
        num_map = {"dos": 2, "tres": 3, "cuatro": 4, "cinco": 5, "seis": 6,
                   "siete": 7, "ocho": 8, "nueve": 9, "diez": 10}
        exp_str = power_n.group(2)
        exp = num_map.get(exp_str, int(exp_str) if exp_str.isdigit() else None)
        if exp:
            return f"{power_n.group(1)}**{exp}"

    # ── Factorial ────────────────────────────────────────────────
    fact1 = re.search(r"factorial de\s*([\d]+)", text)
    if fact1:
        return f"factorial({fact1.group(1)})"
    fact2 = re.search(r"([\d]+)\s*!", text)
    if fact2:
        return f"factorial({fact2.group(1)})"

    # ── MCD / MCM ────────────────────────────────────────────────
    mcd_m = re.search(r"(?:mcd|maximo comun divisor)\s+(?:de\s+)?([\d]+)\s+y\s+([\d]+)", text)
    if mcd_m:
        return f"mcd({mcd_m.group(1)},{mcd_m.group(2)})"
    mcm_m = re.search(r"(?:mcm|minimo comun multiplo)\s+(?:de\s+)?([\d]+)\s+y\s+([\d]+)", text)
    if mcm_m:
        return f"mcm({mcm_m.group(1)},{mcm_m.group(2)})"

    # ── Logaritmo ────────────────────────────────────────────────
    log_base = re.search(r"log(?:aritmo)?\s+(?:base\s+)?([\d\.]+)\s+de\s+([\d\.]+)", text)
    if log_base:
        return f"logb({log_base.group(2)},{log_base.group(1)})"
    log_nat = re.search(r"(?:ln|log(?:aritmo)? natural)\s+de\s+([\d\.]+)", text)
    if log_nat:
        return f"ln({log_nat.group(1)})"
    log10 = re.search(r"log(?:aritmo)?\s+(?:en base 10|base diez|10)?\s+de\s+([\d\.]+)", text)
    if log10:
        return f"log10({log10.group(1)})"

    # ── Trigonometría en grados (más natural) ───────────────────
    trig_patterns = [
        (r"seno\s+(?:de\s+)?([\d\.]+)\s*(?:grados?)?", "sind"),
        (r"coseno\s+(?:de\s+)?([\d\.]+)\s*(?:grados?)?", "cosd"),
        (r"tangente\s+(?:de\s+)?([\d\.]+)\s*(?:grados?)?", "tand"),
        (r"sen\s*\(\s*([\d\.]+)\s*\)", "sind"),
        (r"cos\s*\(\s*([\d\.]+)\s*\)", "cosd"),
        (r"tan\s*\(\s*([\d\.]+)\s*\)", "tand"),
    ]
...
def extract_math_expression(text: str) -> Optional[str]:
    """
    Motor de extracción matemática mejorado.
    1. Detecta operaciones especiales en español
    2. Convierte palabras numéricas
    3. Busca expresiones numéricas directas
    """
    cleaned = text.strip()

    # Eliminar prefijos de pregunta
    for prefix in [
        "cuanto es", "cuanto da", "cuanto resulta", "cuanto seria", "cuanto son",
        "calcula", "calculame", "calcular", "resuelve", "resuelveme",
        "cual es el resultado de", "cual es el valor de",
        "dime cuanto es", "dime el resultado de",
        "puedes calcular", "puedes resolver",
    ]:
        cleaned = re.sub(rf"\b{re.escape(prefix)}\b\s*", "", cleaned)

    # 1) Intentar operaciones especiales
    special = _try_special_operations(cleaned)
    if special:
        return special

    # 2) Convertir palabras a expresión
    word_expr = words_to_expression(cleaned)

    # 3) Buscar expresión numérica (básica o con funciones)
    func_match = re.search(r"(sqrt|cbrt|abs|log|ln|sin|cos|tan|factorial|fact|exp|mcd|mcm|C|P)\s*\(", word_expr)
    if func_match:
        # Hay una función matemática, extraer desde ahí
        start = func_match.start()
        return word_expr[start:].strip()

    # 4) Expresión numérica directa (con operadores)
    expr_match = re.search(
        r"-?[\d\.\,]+\s*[\+\-\*\/x×÷\^%\*]{1,2}\s*-?[\d\.\,]+(?:\s*[\+\-\*\/x×÷\^%\*]{1,2}\s*-?[\d\.\,]+)*",
        word_expr
    )
    if expr_match:
        result = expr_match.group(0)
...
class BaroBrain:
    ...
    async def process(self, raw_text: str) -> BrainResponse:
        ...
        if matches_any(CALC_PATTERNS, text):
            # Primero intentar operaciones avanzadas en lenguaje natural
            advanced = self._handle_advanced_calc(text)
            if advanced:
                return advanced
            # Luego intentar extracción de expresión matemática
            maybe_expr = extract_math_expression(text)
            if maybe_expr:
                return self._handle_calculation(maybe_expr)
            # Si hay patrones pero no se pudo extraer, pedir más detalle
            if re.search(r"\b(calcula|resuelve|suma|resta|multiplica|divide)\b", text):
                return self._finish(
                    "calc_need_expr",
                    "Para calcular necesito la operación completa. Por ejemplo: 'calcula 25 * 4' o 'raíz cuadrada de 144'. ¿Puedes darme los números y la operación?"
                )
        ...
        # 36) Fallback matemático avanzado
        advanced_fallback = self._handle_advanced_calc(text)
    ...
    def _handle_calculation(self, expr: str) -> BrainResponse:
        # Manejo especial de tokens de operación avanzada
        ...
        try:
            ...
            msg = f"Ocurrió un error inesperado al calcular. Intenta reformular la operación."
            return self._finish("calc_error", msg)
    ...
    def _handle_advanced_calc(self, raw_text: str) -> Optional[BrainResponse]:
        """
        Detecta y ejecuta operaciones matemáticas avanzadas en lenguaje natural
        ANTES de intentar extract_math_expression.
        Devuelve BrainResponse si resuelve, None si no aplica.
        """
        text = raw_text  # ya normalizado

        # ── Estadística ──────────────────────────────────────────
        stats_match = re.search(
            r"(?:promedio|media|mediana|moda|desviacion|varianza|estadistica)\s+(?:de|de los?|de las?)?\s*([\d\s,\.y]+)",
            text
        )
        if stats_match:
            nums_raw = stats_match.group(1)
            nums = [float(n.replace(",", ".")) for n in re.findall(r"[\d]+(?:[.,]\d+)?", nums_raw)]
            if len(nums) >= 2:
                try:
                    msg = calculate_statistics(nums)
                    return self._finish("calculation", msg, data={"type": "stats"}, action="show_calc_card")
                except CalculatorError as e:
                    return self._finish("calc_error", str(e))

        # ── IMC ──────────────────────────────────────────────────
        imc_match = re.search(
            r"(?:imc|indice de masa corporal)[^\d]*([\d\.]+)\s*(?:kg|kilos|kilogramos)?\s*(?:y|,|con)?\s*([\d\.]+)\s*(?:m|cm|metros?)?",
            text
        )
        if imc_match:
            try:
                w, h = float(imc_match.group(1)), float(imc_match.group(2))
        ...
        # ── Interés simple ────────────────────────────────────────
        interest_simple = re.search(
            r"interes simple.*?capital\s*([\d\.]+).*?tasa\s*([\d\.]+).*?(?:tiempo|anos?|años?)\s*([\d\.]+)|"
            r"capital\s*([\d\.]+).*?tasa\s*([\d\.]+).*?(?:tiempo|anos?|años?)\s*([\d\.]+).*?interes simple",
            text
        )
        if interest_simple:
        ...
        # ── Interés compuesto ─────────────────────────────────────
        interest_comp = re.search(r"interes compuesto", text)
        if interest_comp:
            nums = _extract_numbers_from_text(text)
            if len(nums) >= 3:
                n = nums[3] if len(nums) >= 4 else 12
                try:
                    msg = calculate_compound_interest(nums[0], nums[1], nums[2], n)
                    return self._finish("calculation", msg, data={"type": "compound"}, action="show_calc_card")
                except CalculatorError as e:
                    return self._finish("calc_error", str(e))

        # ── Conversión de temperatura ─────────────────────────────
        temp_conv = re.search(
            r"([\d\.]+)\s*(?:grados\s+)?(celsius|fahrenheit|kelvin|centigrados|°c|°f|°k|c|f|k)\s+"
            r"(?:a|en)\s+(celsius|fahrenheit|kelvin|centigrados|°c|°f|°k|c|f|k)",
            text
        )
        if temp_conv:
            try:
                val = float(temp_conv.group(1))
                msg = convert_temperature(val, temp_conv.group(2), temp_conv.group(3))
                return self._finish("calculation", msg, data={"type": "temp_convert"}, action="show_calc_card")
            except CalculatorError as e:
                return self._finish("calc_error", str(e))

        # Variantes de temperatura
        temp2 = re.search(r"(?:de\s+)?(celsius|fahrenheit|kelvin)\s+(?:a|en)\s+(celsius|fahrenheit|kelvin).*?([\d\.]+)", text)
        if temp2:
            try:
                msg = convert_temperature(float(temp2.group(3)), temp2.group(1), temp2.group(2))
                return self._finish("calculation", msg, data={"type": "temp_convert"}, action="show_calc_card")
            except CalculatorError as e:
                return self._finish("calc_error", str(e))
        ...
        len_conv = re.search(
            r"([\d\.]+)\s*(km|metros?|cm|mm|millas?|pies|pie|pulgadas?|yardas?|nm)\s+"
            r"(?:a|en)\s+(km|metros?|cm|mm|millas?|pies|pie|pulgadas?|yardas?|nm)",
            text
        )
        if len_conv:
            unit_map = {
                "metros": "m", "metro": "m", "kilómetros": "km", "kilómetro": "km",
                "centímetros": "cm", "centímetro": "cm", "milímetros": "mm", "milímetro": "mm",
                "millas": "millas", "milla": "millas", "pies": "pies", "pie": "pies",
                "pulgadas": "pulgadas", "pulgada": "pulgadas",
                "yardas": "yardas", "yarda": "yardas",
            }
            try:
                val = float(len_conv.group(1))
                fu = unit_map.get(len_conv.group(2), len_conv.group(2))
                tu = unit_map.get(len_conv.group(3), len_conv.group(3))
                msg = convert_length(val, fu, tu)
                return self._finish("calculation", msg, data={"type": "len_convert"}, action="show_calc_card")
            except CalculatorError as e:
                return self._finish("calc_error", str(e))

        # ── Conversión de peso ────────────────────────────────────
        weight_conv = re.search(
            r"([\d\.]+)\s*(kg|g|mg|gramos?|kilos?|libras?|lb|onzas?|oz|toneladas?|ton)\s+"
            r"(?:a|en)\s+(kg|g|mg|gramos?|kilos?|libras?|lb|onzas?|oz|toneladas?|ton)",
            text
        )
        if weight_conv:
            wmap = {
                "kilos": "kg", "kilo": "kg", "kilogramos": "kg",
                "gramos": "g", "gramo": "g",
                "miligramos": "mg", "miligramo": "mg",
                "libras": "lb", "libra": "lb",
                "onzas": "oz", "onza": "oz",
                "toneladas": "ton", "tonelada": "ton",
            }
            try:
                val = float(weight_conv.group(1))
                fu = wmap.get(weight_conv.group(2), weight_conv.group(2))
                tu = wmap.get(weight_conv.group(3), weight_conv.group(3))
                msg = convert_weight(val, fu, tu)
                return self._finish("calculation", msg, data={"type": "weight_convert"}, action="show_calc_card")
            except CalculatorError as e:
                return self._finish("calc_error", str(e))

        # ── Ecuación cuadrática ───────────────────────────────────
        quad = re.search(
            r"(?:ecuacion cuadratica|ecuacion de segundo grado|resolver)\s+"
            r"(-?[\d\.]+)\s*x\s*(?:\^|al cuadrado|2)?\s*([+\-]\s*[\d\.]+)\s*x\s*([+\-]\s*[\d\.]+)",
            text
        )
        if quad:
            try:
                a, b, c = float(quad.group(1)), float(quad.group(2).replace(" ","")), float(quad.group(3).replace(" ",""))
                msg = solve_quadratic(a, b, c)
                return self._finish("calculation", msg, data={"type": "quadratic"}, action="show_calc_card")
            except Exception:
                pass

        # ── Fibonacci ─────────────────────────────────────────────
        fib = re.search(r"(?:fibonacci|fib)\s*(?:de|numero|#|n=)?\s*(\d+)", text)
        if fib:
            try:
                msg = nth_fibonacci(int(fib.group(1)))
                return self._finish("calculation", msg, data={"type": "fibonacci"}, action="show_calc_card")
            except CalculatorError as e:
                return self._finish("calc_error", str(e))

        # ── Número primo ─────────────────────────────────────────
        prime_check = re.search(r"(?:es primo|primalidad|primo)\s+(?:el\s+)?(\d+)|(\d+)\s+(?:es primo|primo\?)", text)
        if prime_check:
            n = int(prime_check.group(1) or prime_check.group(2))
            msg = is_prime(n)
            return self._finish("calculation", msg, data={"type": "prime"}, action="show_calc_card")

        # ── Factorización prima ───────────────────────────────────
        factorize = re.search(r"(?:factores? primos?|factorizacion|factorizar)\s+(?:de\s+)?(\d+)", text)
        if factorize:
            try:
                msg = prime_factors(int(factorize.group(1)))
                return self._finish("calculation", msg, data={"type": "prime_factors"}, action="show_calc_card")
            except CalculatorError as e:
                return self._finish("calc_error", str(e))

        # ── Regla de tres ─────────────────────────────────────────
        r3 = re.search(r"regla de tres.*?([\d\.]+).*?([\d\.]+).*?([\d\.]+)", text)
        if r3:
            try:
                a, b, c = float(r3.group(1)), float(r3.group(2)), float(r3.group(3))
                inverse = "inversa" in text
                msg = rule_of_three(a, b, c, inverse)
                return self._finish("calculation", msg, data={"type": "rule3"}, action="show_calc_card")
            except CalculatorError as e:
                return self._finish("calc_error", str(e))

        # ── Área y perímetro ─────────────────────────────────────
        area_m = re.search(
            r"(?:area|perimetro|volumen|superficie)\s+(?:de\s+(?:un\s+|la\s+|el\s+)?)?(\w+).*?([\d\.]+)(?:\s*(?:y|,)\s*([\d\.]+))?(?:\s*(?:y|,)\s*([\d\.]+))?",
            text
        )
        if area_m:
            shape = area_m.group(1)
            nums = [float(area_m.group(i)) for i in range(2, 5) if area_m.group(i)]
            try:
                msg = area_perimeter(shape, *nums)
                return self._finish("calculation", msg, data={"type": "geometry"}, action="show_calc_card")
            except CalculatorError as e:
                return self._finish("calc_error", str(e))

        # ── Progresión aritmética ─────────────────────────────────
        arith_prog = re.search(
            r"progresion aritmetica.*?(?:primer termino|a1|a₁)\s*=?\s*([\d\.-]+).*?(?:razon|diferencia|d)\s*=?\s*([\d\.-]+).*?(?:terminos|n)\s*=?\s*(\d+)",
            text
        )
        if arith_prog:
            try:
                msg = arithmetic_sequence(float(arith_prog.group(1)), float(arith_prog.group(2)), int(arith_prog.group(3)))
                return self._finish("calculation", msg, data={"type": "arith_seq"}, action="show_calc_card")
            except CalculatorError as e:
                return self._finish("calc_error", str(e))

        # ── Progresión geométrica ─────────────────────────────────
        geo_prog = re.search(
            r"progresion geometrica.*?(?:primer termino|a1|a₁)\s*=?\s*([\d\.-]+).*?(?:razon|r)\s*=?\s*([\d\.-]+).*?(?:terminos|n)\s*=?\s*(\d+)",
            text
        )
        if geo_prog:
            try:
                msg = geometric_sequence(float(geo_prog.group(1)), float(geo_prog.group(2)), int(geo_prog.group(3)))
                return self._finish("calculation", msg, data={"type": "geo_seq"}, action="show_calc_card")
            except CalculatorError as e:
                return self._finish("calc_error", str(e))
